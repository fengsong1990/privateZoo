...
class PreviewPhotoCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        //滑动手势
        //tapswipeDown
        let swipeDown = UISwipeGestureRecognizer.init(target: self, action: #selector(tapswipeDown))
        swipeDown.direction = .down
        self.addGestureRecognizer(swipeDown)
        //长按
        let longTap = UILongPressGestureRecognizer.init(target: self, action: #selector(tapLong(_:)))
    ...
    func refreshUI(phAsset :PHAsset) {
        ...
        let sharedManager = FS_AssetManager.share
         sharedManager.requestPreviewImage(for: phAsset, resultHandler: { [unowned self](image, _) in
    ...
    @objc func tapLong(_ ges:UILongPressGestureRecognizer) {
        ...
        if ges.state == .began{
            ...
            showMessage(message: "保存成功", toView: self, delayTime: 2.0)
        }
    }
    
    
    func showMessage(message:String,toView:UIView,delayTime:CGFloat) {
        
        let hud = MBProgressHUD.showAdded(to: toView, animated: true)
        hud.label.text = message
        hud.mode = .indeterminate
        hud.removeFromSuperViewOnHide = true
        hud.hide(animated: true, afterDelay: TimeInterval(delayTime))
    }
    
    @objc func tapswipeDown(){
